                          ...
                          timeSplit = 10)
{
  ...
  for(iTime in 1:timeFrames)
  {
    ...
    graphics::abline(v = z * timeStep / timeSplit,
                     col = "gray80", lty = 2)
    graphics::points(var~time,
                     type = "o", pch = 20, lwd = 1, col = "gray20")
    if(fleck.data != FALSE){
      graphics::points(baseline1 ~ baselineTime1,
                       data = fleck.data,
                       type = "p", pch = 20, col = "chartreuse3")
      graphics::points(baseline2 ~ baselineTime2,
                       data = fleck.data,
                       type = "p", pch = 20, col = "chartreuse3")
      graphics::points(peak ~ peakTime,
                       data = fleck.data,
    ...
    plot(-500,
         ylab = "",
         ylim = c(min(dev), max(dev)),
         xlim = c(minTime, maxTime),
         xaxt = "n", yaxt = "n")
    graphics::abline(v = z * timeStep / timeSplit,
                     col = "gray80", lty = 2)
    graphics::abline(h = 0, col = "antiquewhite4")
    graphics::points(dev ~ head(seq(0, timeTot, timeStep / timeSplit), -1),
                     type = "l", lwd = 1)
    graphics::axis(side = 2, las = 2, font = 2, cex.axis = 0.8)
    graphics::axis(side = 1, font = 2)
    graphics::mtext(outer = T, side = 1, text = "Time (s)",
                    cex = 1.2, line = 2.2, font = 2)
    graphics::mtext(outer = F, side = 2, text = "1st derivative",
                    cex = 1.2, line = 3, font = 2)
  }
}

#' @rdname plot_ts_fleck
#'
plot_ts_fleck_ez <- function(time,
                             var,
                             zeroes,
                             fleck.data = FALSE,
                             timeSplit = 10)
