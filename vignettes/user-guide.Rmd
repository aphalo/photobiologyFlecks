...
Instead of splitting the data as in the section above, an indexing vector
can be used to do computations in place with fast functions from R package
'collapse'. Given the size of the data sets this could be worthwhile.

```{r}
# generate a vector suitable for grouping
chunks.grp <-
...
```{r}
# extract one chunk, 15 min at 0.05 s
chunk <- chunks.ls[[1]]
range(chunk$time) |> as.numeric() |> diff() # seconds

# time into chunk in seconds as numeric
chunk$tod <- as_tod(chunk$time, unit.out = "seconds")
chunk$chunk.time <- chunk$tod - chunk$tod[1]
range(chunk$chunk.time)

Z <- find_zeros(time = chunk$chunk.time,
                var = chunk$Q_PAR)

sun_flecks.df <- find_flecks(time = chunk$chunk.time,
                             var = chunk$Q_PAR,
                             minTime = 0.001, 
                             minAmp = 0, 
                             minPdiff = 0.00025,
...
sun_flecks.fig <-
  ggplot(data = chunk) +
  geom_line(aes(chunk.time, Q_PAR), linewidth = 0.2, na.rm = TRUE) +
  geom_point(aes(peakTime, peak), data = sun_flecks.df, 
             size = 0.5, colour = "coral2", na.rm = TRUE) +
  geom_point(aes(baselineTime1, baseline1), data = sun_flecks.df,
             size = 0.5, colour = "chartreuse3", na.rm = TRUE) +
  geom_point(aes(baselineTime2, baseline2), data = sun_flecks.df,
             size = 0.5, colour = "chartreuse3", na.rm = TRUE) +
  geom_text(aes(peakTime, peak, label = no), data = sun_flecks.df,
            colour = "coral2",
            position = position_nudge(y = 30), size = 2.7, na.rm = TRUE,
            fontface = "bold")

sun_flecks.fig

sun_flecks.fig + xlim(0, 100)

knitr::kable(sun_flecks.df)
```

```{r}
shade_flecks.df <- find_flecks(time = chunk$chunk.time,
                               var = chunk$Q_PAR,
                               minTime = 0.001, 
                               minAmp = 0, 
                               minPdiff = 0.00025,
...
shade_flecks.fig <-
  ggplot(data = chunk) +
  geom_line(aes(chunk.time, Q_PAR), linewidth = 0.2, na.rm = TRUE) +
  geom_point(aes(peakTime, peak), data = shade_flecks.df,
             size = 0.5, colour = "blue", na.rm = TRUE) +
  geom_point(aes(baselineTime1, baseline1), data = shade_flecks.df,
             size = 0.5, colour = "chartreuse3", na.rm = TRUE) +
  geom_point(aes(baselineTime2, baseline2), data = shade_flecks.df, 
             size = 0.5, colour = "chartreuse3", na.rm = TRUE) +
  geom_text(aes(peakTime, peak, label = no), data = shade_flecks.df, 
            colour = "blue", fontface = "bold",
            position = position_nudge(y = 30), size = 2.7, na.rm = TRUE)

shade_flecks.fig

shade_flecks.fig + xlim(0, 100)

knitr::kable(shade_flecks.df)
```
