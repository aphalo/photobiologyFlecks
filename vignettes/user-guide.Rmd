...
Something seems wrong as I expect a 1 h shift to be needed!

```{r}
system.time(
  statistics.tb <- 
    chunk_summary(chunks.ls, 
                  add.times = TRUE, 
                  tz = "EET",
                  time.shift = 0,
...
ggplot(aes(as_tod(time), PAR_Den_CS)) +
  geom_point(size = 0.1) +
  facet_wrap(facets = vars(date), scales = "free_y")
```

```{r}
statistics.tb |>
  select(solar.time, date, PAR_Den_CS, parameter) |>
  filter(parameter == "mean") |>
ggplot(aes(solar.time, PAR_Den_CS)) +
  geom_point(size = 0.1) +
  facet_wrap(facets = vars(date), scales = "free_y")
```
